...
class SimpleSliderTransformation(
        ...
        page.apply {
            ...
            when {
                position <= -1f -> { // [-Infinity, -1]
                    ...
                    alpha = this@SimpleSliderTransformation.unSelectedItemAlpha
                    scaleX = minScale
                    scaleY = minScale
                }
                position >= -1f && position <= 1f -> { // (0, 1]
                    // page move from right to center.
                    if (this@SimpleSliderTransformation.unSelectedItemAlpha != 1f) {
                        alpha =
                            this@SimpleSliderTransformation.unSelectedItemAlpha + ((1 - abs(position)) *
                                    (1 - this@SimpleSliderTransformation.unSelectedItemAlpha))
                    }
